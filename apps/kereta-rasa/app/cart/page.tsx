...
const Cart = () => {
  const { carts: products, updateCart } = useRootStores();

  const [timer, setTimer] = React.useState<NodeJS.Timeout | null>(null);
  const [totalQty, setTotalQty] = React.useState(0);
  const [totalPrice, setTotalPrice] = React.useState(0);

  const handleChangeNotes = (value: string | undefined, id: number) => {
            ...
            <div className="grid gap-5 col-span-2 order-2 md:order-1">
              {products?.map((product) => (
                <div
                  ...
                  className="w-full border rounded-xl pb-0 overflow-hidden"
                >
                  <div className="flex ">
                  ...
                  <Input
                    placeholder="Catatan ..."
                    className="border-0 bg-transparent mt-3 h-[40px] focus-visible:ring-0"
                    onChange={(e) =>
                      handleChangeNotes(e.target.value, product.id)
                    }
                  />
                </div>
              ))}
